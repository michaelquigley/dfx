...
- **NodeCanvas** (`nodeCanvas.go`, `nodeCanvasGeometry.go`, `nodeCanvasInput.go`, `nodeCanvasWidgets.go`, `nodeCanvasStyle.go`) — zoomable, pannable node-graph editing surface: the app declares the graph and stacking order every frame, the canvas owns view/gesture state and reports intents. Node callbacks are synchronous; widget mouse routing uses the last completed frame's geometry. See `docs/current/node-canvas.md`.
